...
func (r *LocationResolver) PullLocationsByIdentifier(ctx context.Context, countryCode string, partyID string, token string) {
	limit, offset, retries := 500, 0, 0

	if versionEndpoint, err := r.VersionDetailResolver.GetVersionEndpointByIdentity(ctx, "locations", countryCode, partyID); err == nil {
		if requestUrl, err := url.Parse(versionEndpoint.Url); err == nil {
			header := ocpi.NewOCPIRequestHeader(&token, &countryCode, &partyID)
			query := requestUrl.Query()

			if location, err := r.GetLastLocationByIdentity(ctx, countryCode, partyID); err == nil {
			...
			for {
				...
				if response, err := r.OCPIRequester.Do("GET", requestUrl.String(), header, nil); err == nil {
					...
					if err == nil && dto.StatusCode == ocpi.STATUS_CODE_OK {
						r.ReplaceLocationsByIdentifier(ctx, &countryCode, &partyID, dto.Data)

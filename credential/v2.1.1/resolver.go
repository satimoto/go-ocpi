...
import (
	...
	versiondetail "github.com/satimoto/go-ocpi-api/version/v2.1.1"
)
	...
	if payload != nil {
		...
		if version != nil {
			...
			params := db.UpdateCredentialParams{
				...
				ServerToken: util.SqlNullString(uuid.New()),
				Url:         url,
				PartyID:     partyID,
			...
			return nil, rest.OCPIUnsupportedVersion(nil)
		}
	}

	return nil, rest.OCPIRegistrationError(nil)
}
